...
*   **Cache-Busting:** `<script src>` / `<link href>` tags referencing `wwwroot` assets in views MUST use `asp-append-version="true"` (e.g., `<script src="~/js/x.js" asp-append-version="true">`). Kestrel serves static files with only `ETag`/`Last-Modified` and no `Cache-Control`, so browsers (notably Firefox, via heuristic freshness) may serve a stale cached script indefinitely without revalidating — new markup + old JS produces silent no-ops. Content-hash query strings make every JS change a new URL.
*   **Styling:** Bootstrap 5 (CDN via `cdn.jsdelivr.net`). Avoid custom CSS unless absolutely necessary.
*   **Design System:** [`DESIGN.md`](DESIGN.md) at the repo root is the normative UI contract (design tokens + component rules, [google-labs-code/design.md](https://github.com/google-labs-code/design.md) format). **Read it before any view/UI work** and follow it: button intent classes (create=`btn-success`, edit=`btn-sm btn-primary`, row-nav=`btn-sm btn-outline-info`, delete=`btn-sm btn-danger`), one primary action per region, stock Bootstrap only. Tokens deliberately mirror Bootstrap 5.3 defaults — do not introduce new colors, fonts, or custom CSS.
*   **Design System Amendments:** Any change that introduces or alters a UI pattern, interaction, or component not already covered by `DESIGN.md` (e.g., a new expandable-row convention, a new spacing behavior, a new button intent) MUST prompt the user: "this change touches the design system — should DESIGN.md be updated?" If yes, make the amendment in the same change. Never leave the design contract stale after shipping a UI change it doesn't describe.
...
*   **Foreign Key Constraints:** SQLite enforces foreign keys. When deleting a parent row that has child rows referencing it, you **must** delete the child rows first. The current foreign key relationships are:
    ...
    - **Cross-slice deletes are routed via the owning slice's repository.** `SupplementRepository.DeleteAsync` (both overloads) calls `ISupplementNutrientRepository.DeleteBySupplementIdsAsync` and `IPrescribedDoseRepository.DeleteBySupplementIdsAsync`; `FamilyRepository.DeleteAsync` calls `IPrescribedDoseRepository.DeleteByFamilyMemberIdsAsync` — instead of issuing raw SQL against other slices' tables (ADR-0006 cross-slice invariant). When a delete cascade crosses a slice boundary, add a bulk-delete method to the owning slice's repository and call it — never write SQL against another slice's table.
...
7.  **Decision References:** Before design, tracer-bullet, refactoring, or gate decisions, check [`BIBLIOGRAPHY.md`](BIBLIOGRAPHY.md) LLM Prompt Triggers for the governing principle; [`FACTORY.md`](FACTORY.md) is the process contract (DoD per shard); [`VISION.md`](VISION.md) is aspirational direction — not current process.