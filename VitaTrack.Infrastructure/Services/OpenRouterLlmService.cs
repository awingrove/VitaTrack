...
namespace VitaTrack.Infrastructure.Services
{
    public class OpenRouterLlmService : ILlmService
    {
        ...
        private readonly HttpClient _scraperHttp;
            ...
            ILogger<OpenRouterLlmService> logger)
        {
            ...
            _scraperHttp = httpClientFactory.CreateClient("scraper");
            _cfg = cfg;
            _logger = logger;
        }
...
{{
  ...
  ""swapSuggestion"": ""...""
                ...
                if (cleanedContent.StartsWith("```json"))
                {
                    ...
                    if (endIdx >= 0) cleanedContent = cleanedContent.Substring(0, endIdx);
                }
